...
async fn publish(_req: Request) -> Result<impl IntoResponse> {
    ...
    let conn = redis::Connection::open(address).await?;
    ...
    conn.set(
        "spin-example".to_string(),
        "Eureka!".to_string().into_bytes(),
    )
    .await
    .map_err(|_| anyhow!("Error executing Redis set command"))?;

    // Set the Redis key "int-key" to value 0
    conn.set("int-key".to_string(), format!("{:x}", 0).into_bytes())
        .await
        .map_err(|_| anyhow!("Error executing Redis set command"))?;
    let int_value = conn
        .incr("int-key".to_string())
        .await
