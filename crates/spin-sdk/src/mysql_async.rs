...
//!
//! This module uses the async interface introduced in Spin 4.1. If you need to
//! target earlier versions of Spin, use the [blocking interface](crate::mysql).
...
///
/// ```no_run
/// use spin_sdk::mysql_async::{Connection, Decode, ParameterValue};
///
...
///
/// ```no_run
/// use spin_sdk::mysql_async::{Connection, Decode};
///
/// # async fn run() -> anyhow::Result<()> {
/// let db = Connection::open("mysql://root:my_password@localhost/mydb").await?;
///
...
///
/// ```no_run
/// use spin_sdk::mysql_async::{Connection, ParameterValue};
///
/// # async fn run() -> anyhow::Result<()> {
/// let db = Connection::open("mysql://root:my_password@localhost/mydb").await?;
///
...
impl Row {
    ...
    ///
    /// ```no_run
    /// use spin_sdk::mysql_async::{Connection, DbValue};
    ///
    /// # async fn run() -> anyhow::Result<()> {
    /// # let user_id = 0;
    /// let db = Connection::open("mysql://root:my_password@localhost/mydb").await?;
